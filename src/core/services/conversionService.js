/**
 * MindSignal Conversion Service (Node.js / CommonJS version)
 *
 * Central service for converting prompts between different AI video generation engines.
 * Eliminates code duplication across tools and components.
 *
 * Supported engines:
 * - Veo 3.1 (Google)
 * - LtX Pro (LightricksX)
 ...
 */
function convertVeoToLtx(veoPrompt) {
 ...
 */
function convertVeoToLtxString(veoPrompt) {
  const cameraStyleParts = veoPrompt.cameraStyle.split(',').map(s => s.trim());
  const framing = cameraStyleParts[0] || 'medium shot';
  const styleParts = cameraStyleParts.slice(1);
  const outputParts = veoPrompt.output.split(',').map(s => s.trim());
 ...
 */
function convertLtxToVeo(ltxPrompt) {
 ...
 */
function buildVeoFullPrompt(sections) {
  const parts = [];
 ...
 * @returns {string} Formatted LtX Pro prompt
 */
function buildLtxFullPrompt(sections) {
  const parts = [];
 ...
 */
function convert({ prompt, from, to, format = 'object' }) {
  // Validate engines
 ...
 */
function convertBatch(prompts, from, to, format = 'object') {
  return prompts.map(prompt => convert({ prompt, from, to, format }));
}

// CommonJS exports for Node.js tools
module.exports = {
  convertVeoToLtx,
